...
这是一种真实体验，尤其在数据结构和跨层 API 调整时更明显。

| 维度 | C/C++ 常见情况 | Rust 常见情况 |
| --- | --- | --- |
| 改动初期阻力 | 可能先改通接口，问题晚些暴露 | 编译期会立即要求补齐所有权/借用边界 |
| 隐患暴露时机 | 运行期或压测阶段才发现悬垂/UAF/并发别名问题 | 更多问题在编译期暴露 |
| 局部改动体验 | 局部可快速推进，但隐性债可能滞后累积 | 常感觉改一个点要同步修一批签名 |
| 长期维护稳定性 | 依赖规范、评审与测试兜底 | 语言规则直接参与兜底 |

结论：

- Rust 并不是“重构更难”，而是把很多隐性复杂度前置为显性工作。
- 代价是前期改动会更“硬”，收益是后期运行期不确定性通常更低。

## 7. Rust 还有手工内存管理与 `unsafe` 吗
