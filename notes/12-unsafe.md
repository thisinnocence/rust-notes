# 12 - unsafe 边界设计（系统程序员核心）

本章目标：不是教你“多用 unsafe”，而是教你“怎么把 unsafe 缩到最小且可证明”。

## 1. 先给结论

- `unsafe` 不是关闭安全检查，而是把“这段代码的安全前提”转移到你来证明。
...
- 把你在 C 里隐含的前提（对齐、边界、生命周期）写成显式注释与断言。
- unsafe 不是为了“绕过编译器”，而是为了“表达编译器无法自动证明但你能证明的事实”。
...
- 文件顶部给了 `SAFETY TEMPLATE`，可作为你后续 unsafe 注释模板。
