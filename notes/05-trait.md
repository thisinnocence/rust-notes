...
### 3.1 一个类型实现很多 trait，会不会不内聚

这个问题在 Rust 里很常见，重点是“按模块组织能力”，不是把所有能力塞进同一个文件。

| 关注点 | Rust 常见做法 | 对 C/C++ 经验的映射 |
| --- | --- | --- |
| 可发现性 | 用 IDE 查看某类型的全部 `impl`，或用 `cargo doc` 看实现列表 | 类似从头文件/类定义跳到实现，但入口更依赖工具 |
| 内聚性 | 核心领域 trait 放领域模块；跨域能力放适配层模块 | 类似“核心类 + 外挂适配器”分层 |
| 复杂度控制 | 限制“横向滥加 trait”，按职责拆 trait 粒度 | 类似控制 class 职责边界，避免巨型基类 |

结论：

- Rust 的 `trait` 组合并不天然破坏内聚。
- 风险主要来自工程组织不当，而不是 `impl Trait for Type` 语法本身。
