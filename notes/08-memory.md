...
- Rust 没有 Go 那种 GC runtime，不会强制你接受托管堆模型。
...
你提到的 `unwind`，可以理解为“异常回退式退出当前调用栈”：
...
- Rust 要求你把这些前提显式化，因此写法更啰嗦，但边界更清楚。
...
- 性能热点先测量再换分配策略。
- 先局部引入 arena，不要全局一次性替换。
- 把“谁负责释放”写成明确策略，避免内存生命周期失控。

一句话：

- Rust 不是放弃低层控制，而是要求你把低层前提写清楚、验证清楚。
