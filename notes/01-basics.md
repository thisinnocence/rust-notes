...
- 可按如下方式理解：`loop {}` 语义上就是 Rust 版 `while (true)` / `for(;;)`。
...
- `{}` 需要类型实现 `Display` trait；通常需要手写 `impl Display for Type`。
- C 的 `printf("%d", x)` 主要靠格式符和参数类型约定；Rust 是编译期 trait 检查，不匹配直接编译报错。
- 可将其理解为：Rust 把“能不能这样打印”变成了类型系统规则，不是运行时碰运气。
